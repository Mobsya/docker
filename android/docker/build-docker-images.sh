...
echo - mobsya/thymio-dev-env:android${THYMIO_API_LEVEL}-ndk${NDK_VERSION}-qt${QT_BASE_VERSION}.${QT_PATCH_NUMBER}-cmake${CMAKE_VERSION}
...
if [ "$1" == "--push" ]; then
    docker push mobsya/android-sdk:android${QT_API_LEVEL}-ndk${NDK_VERSION} \
        ...
        && docker push mobsya/thymio-dev-env:android${THYMIO_API_LEVEL}-ndk${NDK_VERSION}-qt${QT_BASE_VERSION}.${QT_PATCH_NUMBER}-cmake${CMAKE_VERSION}
else

...
QT_PATCH_NUMBER=2
QT_BUILD_API_LEVEL=29
THYMIO_BUILD_API_LEVEL=24   # SDK API 21 is compatible with Android 5 Lollipop, SDK API 24 is compatible with Android 7.
...
echo --- Installing Android SDK with API level $QT_BUILD_API_LEVEL and $THYMIO_BUILD_API_LEVEL, and NDK version $NDK_VERSION
docker build -t mobsya/android-sdk:android${QT_BUILD_API_LEVEL}-ndk${NDK_VERSION} --build-arg NDK_VERSION=${NDK_VERSION} --build-arg API_LEVEL=${QT_BUILD_API_LEVEL} -f Dockerfile.android-sdk .
docker build -t mobsya/android-sdk:android${THYMIO_BUILD_API_LEVEL}-ndk${NDK_VERSION} --build-arg NDK_VERSION=${NDK_VERSION} --build-arg API_LEVEL=${THYMIO_BUILD_API_LEVEL} -f Dockerfile.android-sdk .
docker build -t mobsya/android-sdk:android${THYMIO_DEPLOYMENT_TARGET_API_LEVEL}-ndk${NDK_VERSION} --build-arg NDK_VERSION=${NDK_VERSION} --build-arg API_LEVEL=${THYMIO_DEPLOYMENT_TARGET_API_LEVEL} -f Dockerfile.android-sdk .

echo --- Building Qt ${QT_BASE_VERSION}.${QT_PATCH_NUMBER} for Android with API level $QT_BUILD_API_LEVEL, and NDK version $NDK_VERSION
if [ "$1" != "--no-qt-build" ]; then
  docker build -t mobsya/qt-builder:android${QT_BUILD_API_LEVEL}-ndk${NDK_VERSION}-qt${QT_BASE_VERSION}.${QT_PATCH_NUMBER} --build-arg NDK_VERSION=${NDK_VERSION} --build-arg API_LEVEL=${QT_BUILD_API_LEVEL} --build-arg QT_BASE_VERSION=${QT_BASE_VERSION} --build-arg QT_PATCH_NUMBER=${QT_PATCH_NUMBER} -f Dockerfile.qt-builder .
fi
...
echo - mobsya/android-sdk:android${QT_BUILD_API_LEVEL}-ndk${NDK_VERSION}
